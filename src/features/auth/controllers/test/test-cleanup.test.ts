...
// test-cleanup.ts reads config.TEST_CLEANUP_SECRET once at module load time
// (`process.env.TEST_CLEANUP_SECRET`, no hardcoded fallback — see config.ts).
// jest.mock is hoisted above these imports, so we set it on the real config
// singleton here rather than asserting against a literal baked into this file.
jest.mock('@root/config', () => {
  const actual = jest.requireActual('@root/config');
  actual.config.TEST_CLEANUP_SECRET = process.env.TEST_CLEANUP_SECRET || 'jest-mock-test-cleanup-secret';
  return actual;
});

const TEST_CLEANUP_SECRET = config.TEST_CLEANUP_SECRET;
...
describe('TestCleanup', () => {
  ...
  it('throws NotFoundError when the auth user does not exist', async () => {
    ...
    const req = mockRequest({ 'x-test-secret': TEST_CLEANUP_SECRET as string }, { authId: AUTH_ID });
    const res: Response = authMockResponse();
