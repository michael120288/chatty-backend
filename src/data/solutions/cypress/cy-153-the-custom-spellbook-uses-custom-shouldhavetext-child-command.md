# The Custom Spellbook — uses custom shouldHaveText child command
...
describe('The Custom Spellbook', () => {
  it('uses custom shouldHaveText child command', () => {
      cy.visitLevel(1);
      cy.get('#featured-item').shouldHaveText('Magic Sword');
    });
});
```
