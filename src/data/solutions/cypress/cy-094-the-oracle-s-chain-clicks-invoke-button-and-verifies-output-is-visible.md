# The Oracle's Chain — clicks invoke button and verifies output is visible
...
describe('The Oracle's Chain', () => {
  it('clicks invoke button and verifies output is visible', () => {
      cy.visit('/pages/level-62/');
      cy.get('#invoke-btn').click();
      cy.get('#reveal-output').should('be.visible');
    });
});
```
...
describe('The Oracle', () => {
  it('clicks invoke button and verifies output is visible', () => {
    cy.visit('/pages/level-62/');
    // Hint: click the element "#invoke-btn"
    // Hint: select "#reveal-output" and assert it is visible
  });
});
```
