# The Dialogue Keeper — dialog handler is set up before any interaction
...
describe('The Dialogue Keeper', () => {
  it('dialog handler is set up before any interaction', () => {
      cy.visit('/pages/level-12/');
      // Must register cy.on before the event fires
      cy.on('window:confirm', (text) => {
        expect(text).to.be.a('string');
        return true;
      });
      cy.get('#confirm-btn').click();
      cy.get('#oracle-result').should('exist');
    });
});
```
...
describe('The Dialogue Keeper', () => {
  it('dialog handler is set up before any interaction', () => {
    ...
    // Hint: select "#oracle-result" and assert it exists in the DOM
  });
});
```
