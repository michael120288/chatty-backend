...
describe('Quest Portal — Full Journey', () => {
  it('completes the full quest registration journey', () => {
    cy.visit('/pages/level-74/');
    // Hint: type "Aria Stormwind" into "#adventurer-name"
    // Hint: select option "Mage" from "#hero-class"
    // Hint: use cy.get()
    // Hint: click the element "#register-btn"
    // Hint: use cy.url()
    // Hint: select "#summary-name" and assert its text equals "Aria Stormwind"
    // Hint: select "#summary-class" and assert its text equals "Mage"
    // Hint: use cy.get()
    // Hint: click the element "#begin-btn"
    // Hint: use cy.url()
    // Hint: use cy.get()
    // Hint: click the element "#claim-reward-btn"
    // Hint: select "#reward-section" and assert it is visible
  });
});
```
