...
export class ReactionCache extends BaseCache {
  ...
  public async savePostReactionToCache(key: string,reaction: IReactionDocument,postReactions:IReaction,type:string,previousReaction:string):Promise<void> {
    try {
  ...
  public async removePostReactionFromCache(key: string,username:string,postReaction:IReaction):Promise<void> {
    try {
      if(!this.client.isOpen){
        await this.client.connect();
      }
